...
const videoMiddleware = store => next => action => {
    ...
    const { pageToken } = store.getState()
        ...
        fetch(`https://www.googleapis.com/youtube/v3/search?key=AIzaSyALXCDcbNJBKRzdvdA8-mzDBQQTRTjKVPU&type=video&part=snippet&maxResults=15&q=${inputValue}`)
        .then(res => res.json())
            .then(data => {
    ...
    if(action.type === NEXT_PAGE) {
        fetch(`https://www.googleapis.com/youtube/v3/search?key=AIzaSyALXCDcbNJBKRzdvdA8-mzDBQQTRTjKVPU&type=video&part=snippet&maxResults=15&q=${inputValue}&pageToken=${pageToken}`)
        .then(res => res.json())
        .then(data => {
